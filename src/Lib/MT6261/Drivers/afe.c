...
void BeepHz(uint32_t freq_hz, uint32_t duration_ms)
{
    // Assume system clock reference for DAC sine gen is 26 MHz
    const uint32_t SYS_CLK_HZ = 26000000;

    // Derive frequency divider value for the given tone frequency
    // freq_out = SYS_CLK / (N × freq_div)
    // Rearranged: freq_div = SYS_CLK / (N × freq_out)
    // We'll assume N = 32 as a typical internal factor (empirical guess)
    uint32_t N = 32;
    uint32_t freq_div = SYS_CLK_HZ / (N * freq_hz);

    if (freq_div > 255) freq_div = 255;      // Clamp to max allowed value
    if (freq_div < 1) freq_div = 1;          // Clamp to minimum

    // Use a fixed amplitude divider for now
    uint32_t amp_div = 25; // Lower = louder, 0 is max amplitude

    // Construct the register value
    AFE_DAC_TEST = VDAC_SINUS | ADAC_SINUS | AMP_DIV(amp_div) | FREQ_DIV(freq_div);

    // Wait for specified duration
    USC_Pause_us(duration_ms * 1000);

    // Stop the tone
    AFE_DAC_TEST &= ~(VDAC_SINUS | ADAC_SINUS);
}

void AFE_Unmute(void)
{
    /*
     * Clear analog mute bits and (optionally) wait for unmute completion.
     * On this platform the downlink analog mutes reside in ABBA_AUDIODL_CON16 bits 0/1
     * (RG_AMUTEL / RG_AMUTER). They appear also reflected logically by AMUTEL/AMUTER in
     * AFE_AMCU_CON1 for the digital path. We clear both analog and digital request bits.
     * If zero-cross (ZCD) is enabled the hardware may ramp; we can poll AFE_MUTE_STA if needed.
     */

    // Clear digital mute request bits if set
    AFE_AMCU_CON1 &= ~(AMUTEL | AMUTER);

    // Clear analog mute bits (if they exist); guard in case register not defined on variant
#ifdef ABBA_AUDIODL_CON16
    ABBA_AUDIODL_CON16 &= ~(RG_AMUTEL | RG_AMUTER);
#endif

    // Optional: wait for UNMUTE_DONE flags (with small timeout to avoid lock)
    uint32_t guard = 10000;
    while (guard--) {
        uint16_t st = AFE_MUTE_STA;
        if ((st & (UNMUTE_DONE_L | UNMUTE_DONE_R)) == (UNMUTE_DONE_L | UNMUTE_DONE_R)) break;
    }
}

void AFE_EnsureOutputPath(void)
{
    // Re-enable DL path & AFE core (idempotent)
    AFE_MCU_CON1 = (AFE_MCU_CON1 & ~AFE_PATH_MASK) | UDSP_DL_ON | A_IF_DL_ON;
    AFE_MCU_CON0 |= AFE_ON;

    // Make sure DACs & HS buffer powered
    ABBA_AUDIODL_CON0 |= RG_AUDDACLPWRUP | RG_AUDDACRPWRUP | RG_AUDHSPWRUP;

    // Max (or desired) analog gains
    ABBA_AUDIODL_CON13 = RG_GAINS(0x1F) | RG_GAINL(0x1F) | RG_GAINR(0x1F);

    // Speaker digital gain
    AFE_SetSpeakerVolume(15); // adjust range as needed
